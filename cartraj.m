...
v = 3; % Speed
delta_max = 2*pi; % max steering angle
k = 2.5; % Gain
% l = 1; % Car length
...
p = zeros(length(T),2); % Position in xc,y plane
p(1,:) = (x0(1))*[sin(xp) cos(xp)]; % Initial position

for i=1:length(T)-1
   % Calculate steering angle
   delta(i) = max(-delta_max,min(delta_max,xc(i,2) + atan2(k*xc(i,1),v)));
   % State derivatives
   xd(i,1) = v*sin(xc(i,2) - delta(i));
   xd(i,2) = -(v*sin(delta(i)))/(l);
   w = xd(i,2);
   % State update
   xc(i+1,1) = xc(i,1)+dt*xd(i,1);
   xc(i+1,2) = xc(i,2)+dt*xd(i,2);
   % Position update
   p(i+1,1) = p(i,1) + dt*v*cos(xc(i,2)-delta(i)-xp);
   p(i+1,2) = p(i,2) + dt*v*sin(xc(i,2)-delta(i)-xp);
end
...
for t=1:300:length(T)
      drawbox(p(t,1),p(t,2),xc(t,2),.3,1);
end
xlabel('x (m)')
ylabel('y (m)')
axis equal
